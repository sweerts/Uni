...
\corr{Es ist noch zu zeigen, dass jede Zerlegung von $\varphi(f)$ die Form $\varphi(f)= \varphi(g)\cdot \varphi(h)$ hat mit $f=g \cdot h$. Nur für diesen Fall wurde Irreduzibilität gezeigt. $-0,5$ P.}
		...
		\begin{align*}
			&\forall a,b\in R\corr{[t]},\varphi(f)=a\cdot b:a\in R\corr{[t]}^*\text{ oder }b\in R\corr{[t]}^* \\
			\overset{f=\varphi^{-1}(\varphi(f))}{\df}&\forall a,b\in R\corr{[t]},\varphi(f)=a\cdot b:f=\varphi^{-1}(\varphi(f))=\varphi^{-1}(a\cdot b)=\varphi^{-1}(a)\cdot\varphi^{-1}(b) \\
			\overset{2.3.3}{\df}&\varphi^{-1}(a)\in R\corr{[t]}^*\text{ oder }\varphi^{-1}(b)\in R\corr{[t]}^* \\
